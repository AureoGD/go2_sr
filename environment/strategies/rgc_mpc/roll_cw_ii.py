...
class RollCW(BaseRGC):
    ...
    def __init__(self, **kwargs):
        ...
        self.nx = 23
        self.nu = 12
        self.ny = 12  # orientation, q FL pos, q FR pos, romega_x
        ...
        # Body orientation
        self.Ca[0:12, self.nx:] = np.eye(12)
        ...
        Qfr = np.diag(np.array([1, 1, 1]))
        Qfr = np.diag(np.array([1, 1, 1]))
        Qrr = np.diag(np.array([1, 1, 1]))
        Qrl = 2 * np.diag(np.array([1, 1, 1]))

        Q = block_diag(Qfr, Qfr, Qrr, Qrl)
        ...
        Rdqrl = 100 * np.diag(np.array([1, 1, 1]))

        R = block_diag(Rdqfr, Rdqfl, Rdqrr, Rdqrl)
        self.R = block_diag(*[R] * self.M)
        # np.array([-0.6, 1.5, -2.0, -0.8, 1.0, -2.6, -0.6, 1.5, -2.0, 0.6, 3.75, -1.5])
        # qrRef = np.array([-0.6, 1.5, -2.0, -0.8, 1.0, -2.6, -0.6, 1.5, -2.0, 0.6, 3.75, -1.5]).reshape(12, 1)
        qrRef = np.array([-0.6, 1.5, -2.0, -0.8, 1.0, -2.6, -0.6, 1.5, -2.0, 0.4, 3.75, -1.5]).reshape(12, 1)
        self.ref = np.tile(qrRef, (self.N, 1))

        # Constraints bounds
        # Max and min joint references
        qr_l = np.array([
    ...
    def update_model(self):
        ...
        cross_fr = self.skew_symmetric_matrix(pc_fr - pc_fr)
        ...
        cross_rr = self.skew_symmetric_matrix(pc_rr - pc_rr)
        ...
        self.Ba[3:6, 0:3] = np.eye(3)
        self.Ba[6:9, 3:6] = np.eye(3)
        self.Ba[9:12, 6:9] = np.eye(3)

        self.Ba[15:18, 0:9] = J_com[:, 0:9]
