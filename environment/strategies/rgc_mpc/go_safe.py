...
class GoSafe(BaseRGC):
    ...
    def __init__(self, **kwargs):
        ...
        qr = np.array([[0.9, 1.4, -2.7, -0.9, 1.4, -2.7, 0.9, 1.4, -2.7, -0.9, 1.4, -2.7]]).transpose()
    ...
    def define_constraints_matrices(self):
        ...
        fr_foot = self.data.oMf[self.model.getFrameId('FR_foot')].translation
        fr_calf = self.data.oMf[self.model.getFrameId('FR_calf_joint')].translation
        fl_foot = self.data.oMf[self.model.getFrameId('FL_foot')].translation
        fl_calf = self.data.oMf[self.model.getFrameId('FL_calf_joint')].translation
        rr_foot = self.data.oMf[self.model.getFrameId('RR_foot')].translation
        rr_calf = self.data.oMf[self.model.getFrameId('RR_calf_joint')].translation
        rl_foot = self.data.oMf[self.model.getFrameId('RL_foot')].translation
        rl_calf = self.data.oMf[self.model.getFrameId('RL_calf_joint')].translation

        # FR -> FL
        c1, c2, dist_fr_fl, n_fr_fl = self.closest_points_segments(fr_calf, fr_foot, fl_calf, fl_foot)
        J_c_fr_fl_1 = self.get_jacobian_at_point(frame_id=self.model.getFrameId('FR_foot'), point_world=c1, q=q)[:, 3:6]
        J_c_fr_fl_2 = self.get_jacobian_at_point(frame_id=self.model.getFrameId('FL_foot'), point_world=c2, q=q)[:, 0:3]

        # FR -> RR
        c1, c2, dist_fr_rr, n_fr_rr = self.closest_points_segments(fr_calf, fr_foot, rr_calf, rr_foot)
        J_c_fr_rr_1 = self.get_jacobian_at_point(frame_id=self.model.getFrameId('FR_foot'), point_world=c1, q=q)[:, 3:6]
        J_c_fr_rr_2 = self.get_jacobian_at_point(frame_id=self.model.getFrameId('RR_foot'), point_world=c2, q=q)[:,
                                                                                                                 9:12]

        # FR -> RL
        c1, c2, dist_fr_rl, n_fr_rl = self.closest_points_segments(fr_calf, fr_foot, rl_calf, rl_foot)
        J_c_fr_rl_1 = self.get_jacobian_at_point(frame_id=self.model.getFrameId('FR_foot'), point_world=c1, q=q)[:, 3:6]
        J_c_fr_rl_2 = self.get_jacobian_at_point(frame_id=self.model.getFrameId('RL_foot'), point_world=c2, q=q)[:, 6:9]

        # FL -> RR
        c1, c2, dist_fl_rr, n_fl_rr = self.closest_points_segments(fl_calf, fl_foot, rr_calf, rr_foot)
        J_c_fl_rr_1 = self.get_jacobian_at_point(frame_id=self.model.getFrameId('FL_foot'), point_world=c1, q=q)[:, 0:3]
        J_c_fl_rr_2 = self.get_jacobian_at_point(frame_id=self.model.getFrameId('RR_foot'), point_world=c2, q=q)[:,
                                                                                                                 9:12]

        # FL -> RL
        c1, c2, dist_fl_rl, n_fl_rl = self.closest_points_segments(fl_calf, fl_foot, rl_calf, rl_foot)
        J_c_fl_rl_1 = self.get_jacobian_at_point(frame_id=self.model.getFrameId('FL_foot'), point_world=c1, q=q)[:, 0:3]
        J_c_fl_rl_2 = self.get_jacobian_at_point(frame_id=self.model.getFrameId('RL_foot'), point_world=c2, q=q)[:, 6:9]

        # RR -> RL
        c1, c2, dist_rr_rl, n_rr_rl = self.closest_points_segments(rr_calf, rr_foot, rl_calf, rl_foot)
        J_c_rr_rl_1 = self.get_jacobian_at_point(frame_id=self.model.getFrameId('RR_foot'), point_world=c1, q=q)[:,
                                                                                                                 9:12]
        J_c_rr_rl_2 = self.get_jacobian_at_point(frame_id=self.model.getFrameId('RL_foot'), point_world=c2, q=q)[:, 6:9]

        J = np.zeros((6, 12), dtype=np.float32)

        J[0, 0:3] = n_fr_fl @ J_c_fr_fl_1
        J[0, 3:6] = -n_fr_fl @ J_c_fr_fl_2

        J[1, 0:3] = n_fr_rr @ J_c_fr_rr_1
        J[1, 6:9] = -n_fr_rr @ J_c_fr_rr_2

        J[2, 0:3] = n_fr_rl @ J_c_fr_rl_1
        J[2, 9:12] = -n_fr_rl @ J_c_fr_rl_2

        J[3, 3:6] = n_fl_rr @ J_c_fl_rr_1
        J[3, 6:9] = -n_fl_rr @ J_c_fl_rr_2

        J[4, 3:6] = n_fl_rl @ J_c_fl_rl_1
        J[4, 9:12] = -n_fl_rl @ J_c_fl_rl_2

        J[5, 6:9] = n_rr_rl @ J_c_rr_rl_1
        J[5, 9:12] = -n_rr_rl @ J_c_rr_rl_2

        dist = (self.d_safe + self.radius * 2) - np.vstack(
            (dist_fr_fl, dist_fr_rr, dist_fr_rl, dist_fl_rr, dist_fl_rl, dist_rr_rl))
