...
class Go2ModelSimMuJoCo():
    ...
    def __init__(self, task_control=None, render=False, strategy_name='unitree', **kwargs):
        """
        Args:
            task_control: (Optional) Override for controller
            render (bool): Enable MuJoCo viewer
            strategy_name (str): 'rgc' or 'unitree' - determines which controller to load
        """
        ...
        # We will set this via the strategy selection below
        ...
        # === 1. Load MuJoCo model (Updated Path to 'assets') ===
        ...
        # === 2. Initialize Pinocchio model (Updated Path to 'assets') ===
        ...
        if os.path.exists(urdf_path):
            ...
            print(f"Warning: URDF file not found at {urdf_path}. Pinocchio model not loaded.")
        ...
        # === 3. Initialize Strategy Based on Name ===
        self.base_rgc = BaseRGC(**config)

        if self.task_control is None:
            if kwargs.get('env_id', 0) == 0:
                print(f"[Go2Sim] Initializing Strategy: {strategy_name}", flush=True)

            if strategy_name == 'rgc':
                # The Complex RGC-MPC Scheduler
                self.task_control = SchedulerRGCMPC(**config)

            elif strategy_name == 'unitree':
                # The Default Unitree Logic
                self.task_control = UnitreeSelfRighting(**config)

            else:
                print(f"[Go2Sim] Warning: Unknown strategy '{strategy_name}'. Defaulting to Unitree.")
                self.task_control = UnitreeSelfRighting(**config)
