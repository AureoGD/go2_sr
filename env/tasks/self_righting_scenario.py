...
class SelfRightingScenario(BaseTaskScenario):
    ...
    def __init__(self, current_difficulty=1, config=None):
        ...
        # Base distributions
        # =========================
        self.q0_base = np.array([-0.2, 2.0, -1.65, -0.6, 1.86, -1.65,
                                 -0.5, 1.06, -1.0, 0.25, 1.36, -1.05])
        ...
        # =========================
        self.ends_data = {
            1: {"b_z": 0.10, "rpy": np.array([-3.14, -0.08, 0.0]),
                "q": np.array([0.66, 1.40, -2.61, -0.66, 1.40, -2.61,
                               0.66, 1.40, -2.61, -0.66, 1.40, -2.61])},

            2: {"b_z": 0.11, "rpy": np.array([-3.13, -0.01, -0.05]),
                "q": np.array([-0.51, 1.50, -2.01, -0.80, 1.01, -2.60,
                               -0.48, 1.50, -2.01, -0.69, 4.29, -2.29])},

            3: {"b_z": 0.18, "rpy": np.array([1.54, 0.0, -0.01]),
                "q": np.array([0.29, 1.49, -2.00, -0.60, 1.30, -2.60,
                               0.30, 1.51, -2.00, 0.89, 3.75, -1.50])},

            4: {"b_z": 0.18, "rpy": np.array([1.24, 0.0, 0.0]),
                "q": np.array([0.33, 1.52, -2.01, -0.58, 0.90, -1.54,
                               0.33, 1.50, -2.01, -0.57, 0.93, -1.50])},

            5: {"b_z": 0.13, "rpy": np.array([0.01, -0.02, -0.02]),
                "q": np.array([-0.0, 1.41, -2.72, -0.02, 1.40, -2.72,
                               -0.02, 1.44, -2.73, -0.01, 1.39, -2.73])},

            7: {"b_z": 0.16, "rpy": np.array([-2.68, -0.0, 0.01]),
                "q": np.array([0.80, 1.01, -2.60, 0.57, 1.50, -2.01,
                               -0.35, 4.16, -2.40, 0.52, 1.50, -2.01])},

            8: {"b_z": 0.18, "rpy": np.array([-1.54, 0.0, 0.02]),
                "q": np.array([0.60, 1.30, -2.60, -0.29, 1.49, -2.00,
                               -0.89, 3.75, -1.50, -0.30, 1.51, -2.00])},

            9: {"b_z": 0.18, "rpy": np.array([-1.24, 0.0, 0.02]),
                "q": np.array([0.58, 0.90, -1.54, -0.33, 1.50, -2.00,
                               0.57, 0.93, -1.50, -0.33, 1.50, -2.01])}
        }
    ...
    def sample(self):
        ...
        return {"q0": data["q"].copy(), "r0": r0, "b0": b0}
