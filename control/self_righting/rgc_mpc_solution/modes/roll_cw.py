...
class RollCW(BaseRGCController):
    ...
    def __init__(self, robot_states, **kwargs):
        ...
        qr = np.array([0.8, 4, -1.8]).reshape(3, 1)
        ...
        R_pivot = np.diag(np.array([10, 100, 100]))
        R_fixed = np.diag(np.array([1, 1, 1]))
        R_rear = 0.1 * np.diag(np.array([1, 1, 1]))
        ...
        self.Is = np.concatenate((np.identity(3), np.zeros((3, 3)), np.identity(3), np.identity(3)), axis=1)
    ...
    def update_model(self):
        ...
        self.contacts[0, :] = self.pin_engine.frame_pos("RR", "thigh")
        self.contacts[1, :] = self.pin_engine.frame_pos("RR", "foot")
        self.contacts[2, :] = self.pin_engine.frame_pos("FR", "foot")
        self.contacts[3, :] = self.pin_engine.frame_pos("FR", "thigh")
        self.contacts[4, :] = self.pin_engine.frame_pos("RL", "foot")
    ...
    def build_soft_constraint_matrices(self):
        ...
        self.Ccs[0:5, :] = -pyramid_fric_matrix[0:5, 0:3] @ self.Jinv[9:12, 9:12] @ self.Cch[21:24, :]
        self.Ccs[5, :] = -n_l[1] @ (self.Jinv[0:3, 0:3] @ self.Cch[12:15, :])
        self.Ccs[6, :] = -n_l[2] @ (self.Jinv[6:9, 6:9] @ self.Cch[18:21, :])
