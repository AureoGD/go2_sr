...
class PrepareCW(BaseRGCController):
    ...
    def __init__(self, robot_states, **kwargs):
        ...
        # Reference
        # ----------------------------------------
        qr = np.array([[ -1.05, 1.50, -2.30,
                         -0.00, 1.00, -2.60,
                         -1.05, 1.50, -2.30,
                         -1.02, 4.15, -2.20]]).transpose() 
        ...
        wcs = np.vstack((self.wcs_collision * np.ones((6, 1)),
                         np.array([[self.wcs_plane]]),
                         self.wcs_grf * np.ones((2, 1))))
    ...
    def build_soft_constraint_matrices(self):
        ...
        p_k0 = self.pin_engine.frame_pos('RL', 'calf')
        Jk = self.pin_engine.linear_leg_jacobian('RL', 'calf')
        g = n_w @ Jk
        Ccs[6, 9:12] = g
        knee_lower = lb - n_w @ p_k0 + g @ self.x[9:12]

        f_cte_l = np.array([-np.inf, -np.inf])
        f_cte_u = np.array([np.inf, np.inf])

        J = self.pin_engine.linear_leg_jacobian("FR", "thigh")
        J_inv = np.linalg.pinv(J).T
        self.rs.force_shoulder[0] = -J_inv @ self.cs.tau[0:3]
        if np.linalg.norm(self.rs.force_shoulder[0]) > 30:
            self.fr_count += 1

        if self.fr_count > 5:
            F = -J_inv @ self.Cch[12:15, :]
            Ccs[7, :] = F[2, :].copy()
            f_cte_l[0] = 30
            f_cte_u[0] = 35
        else:
        ...
        J = self.pin_engine.linear_leg_jacobian("RR", "thigh")
        J_inv = np.linalg.pinv(J).T
        self.rs.force_shoulder[2] = -J_inv @ self.cs.tau[6:9]
        if np.linalg.norm(self.rs.force_shoulder[2]) > 30:
            F = -J_inv @ self.Cch[18:21, :]
            Ccs[8, :] = F[2, :].copy()
            f_cte_l[1] = 30
            f_cte_u[1] = 35
        else:
